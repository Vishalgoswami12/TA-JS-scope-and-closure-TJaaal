...
```js
function multiMap(arrVals, arrVals2) {
  let newObj={};
  for(let i=0; i<arrVals.length;i++){
   let element=arrVals[i];
    newObj[element] = [];
    for(let j=0; j<arrVals2.length;j++){
    newObj[element].push(arrVals2[j](element))
  }
  }
  return newObj;
}
...
function objOfMatchesWithArray(array1, array2, callback) {
  ...
  for(let i=0; i<array1.length;i++){
   let element=array1[i];
    ...
    newObj[element].push(array2[j](element))
  }
  }
  return newObj;
}
...
function schedule(callback,allTime) {
  if(callback.length!==allTime.length){
